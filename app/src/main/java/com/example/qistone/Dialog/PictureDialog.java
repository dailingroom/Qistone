...
public class PictureDialog implements View.OnClickListener{
    ...
    private RelativeLayout Picture_cancle;
    //对话框上事件的解决
    ...
    public PictureDialog(Context context,Activity activity) {
    ...
    public PictureDialog builder() {
        ...
        win.setContentView(R.layout.dialog_picture);
        camera=win.findViewById(R.id.camera);
        album=win.findViewById(R.id.album);
        Picture_cancle=win.findViewById(R.id.Picture_cancle);
        camera.setOnClickListener(this);
        album.setOnClickListener(this);
        Picture_cancle.setOnClickListener(this);
        /*
    ...
    private void startCamera() {
        ...
        if(Build.VERSION.SDK_INT >= 24) {
            photoUri = FileProvider.getUriForFile(context, "com.example.qistone.FileProvider", file);
        } else {
    ...
    private void cropPhoto(Uri inputUri) {
        ...
        //Toast.makeText(activity,inputUri.toString(),Toast.LENGTH_SHORT).show();
        ...
        switch (requestCode) {
            ...
            case TAKE_PHOTO_PERMISSION_REQUEST_CODE:
                ...
                if(grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    ...
                    Toast.makeText(activity, "拍照权限被拒绝", Toast.LENGTH_SHORT).show();
                }
                break;
            // 打开相册选取：
            case WRITE_SDCARD_PERMISSION_REQUEST_CODE:
                if(grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    ...
                    Toast.makeText(activity, "读写内存卡内容权限被拒绝", Toast.LENGTH_SHORT).show();
                }
                break;
        }
    }
     ...
     */
    public Bitmap onActivityResult(int requestCode, int resultCode, Intent data,int RESULT_OK) {
        if (resultCode == RESULT_OK) {
            ...
            switch (requestCode) {
                ...
                case CROP_PHOTO_REQUEST_CODE:
                    File file = new File(Objects.requireNonNull(photoOutputUri.getPath()));
                    if (file.exists()) {
...
//                        file.delete(); // 选取完后删除照片
                        ...
                        Toast.makeText(activity, "找不到照片", Toast.LENGTH_SHORT).show();
                    }
                    break;
            }
        }
